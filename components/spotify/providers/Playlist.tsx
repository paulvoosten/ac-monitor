...
export const PlaylistProvider: React.FC<{ id: string }> = ({ id, children }) => {
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => {
      mutate(async data => {
        if (!data) return data;
        const playlist: Playlist = await fetch(`/api/spotify/queue/${data.id}`).then(response =>
          response.json(),
        );
        if ('error' in playlist) return data;
        let queued = Math.max(
          data.tracks.findIndex(track => track.state === 'queued'),
          0,
        );
        const currentUris = data.tracks.map(track => track.uri);
        playlist.tracks.forEach(track => {
          if (!currentUris.includes(track.uri)) {
            data.tracks.splice(getRandomNumber(queued, data.tracks.length), 0, track);
          }
        });
        return data;
      }, false);
  ...
  return (
    <PlaylistContext.Provider value={{ data, error, loading: !error && !data, mutate }}>
      {children}
